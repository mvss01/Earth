function show_description(job_id){
    ...
    var html = '<button class="col-md-12" style="outline: none; border: 1px solid #dbdbdb; background: none; border-radius: 20px; padding: 20px;">'
            ...
            if(session.permission == "User"){
                html += '<a href="/opportunities/register/' + job_description.id + '" class="btn btn-primary text-white col-md-7 mt-3" style="text-decoration: none; float: left; ">Candidatar-se</a>'
                html += '<a href="/favorites/register/' + job_description.id + '" class="btn btn-outline-secondary col-md-4 mt-3" style="text-decoration: none; float: right;">Salvar</a>'
            }else{
                html += '<a href="/jobs/edit/' + job_description.id + '" class="btn btn-primary btn-block text-white col-md-12 mt-3" style="text-decoration: none; float: left; ">Editar</a>'
            }
            html += '</button>'
